// Host harness: drives the REAL firmware controllers with the NEXUS_SIM pitch
// timeline on a virtual clock, decodes the MIDI they emit, reports per phase,
// and asserts the expected pitch-bend behavior. Built host-side (not cross).
//
//   cmake -S test -B test/build && cmake --build test/build
//   ./test/build/host_sim          # report + assertions (exit != 0 on failure)
...
// Decoded events
// ---------------------------------------------------------------------------
struct ev { uint32_t t; char kind; int a, b; };   // R reset, B pb(a=val),
                                                   // C cc(a=num,b=val), P pc(a=val)
static std::vector<ev> events;

// Host-only real-data feed (set by --feed; consumed by sample() in nexus_sim.hpp)
namespace nexus_sim { const int16_t* g_feed = nullptr; uint32_t g_feed_len = 0; }
...
static pbwin pb_in(uint32_t t0, uint32_t t1)
{
   ...
   for (const ev& e : events)
      if (e.kind=='B' && e.t>=t0 && e.t<t1) {
         ...
         w.last=e.a; if (e.a<w.lo) w.lo=e.a; if (e.a>w.hi) w.hi=e.a; ++w.count;
      }
...
int main(int argc, char** argv)
{
   ...
   if (feedfile)                                    // replay logged ADC, 1 sample/ms
   {
      ...
      for (uint32_t ms = 0; ms < len; ++ms) {
         ...
         // Linearly interpolate between logged samples to 1 ms. The diag is decimated
         // (~10-100 ms), so step-holding would replay a staircase whose jumps trip the
         // firmware's slew gate on fast ramps -- reconstruct the real smooth arm motion.
         if (pi + 1 < tv.size() && tv[pi+1] > tv[pi]) {
            double f = (t - tv[pi]) / (tv[pi+1] - tv[pi]);
            g_feedbuf[ms] = (int16_t)(av[pi] + (av[pi+1] - av[pi]) * f + 0.5);
         } else {
   ...
   setup();
   pitch_bend.startup_holdoff = holdoff;   // override the 2500 ms production default for tests
   ...
   ph phases[] = {
      {0,           T_REST1,      "rest1"},     {T_REST1,    T_DIVE,       "dive_down"},
      {T_DIVE,      T_DIVE_HOLD,  "dive_hold"}, {T_DIVE_HOLD,T_DIVE_END,   "dive_up"},
      {T_DIVE_END,  T_REST2,      "rest2"},     {T_REST2,    T_PULL,       "pull_up"},
      {T_PULL,      T_PULL_HOLD,  "pull_hold"}, {T_PULL_HOLD,T_PULL_END,   "pull_down"},
      {T_PULL_END,  T_REST3,      "rest3"},     {T_REST3,    T_WOBBLE_END, "wobble"},
      {T_WOBBLE_END,T_REST4,      "rest4"},     {T_REST4,    T_UP1_H,      "up1_hold"},
      {T_UP1_H,     T_DN1_H,      "dn1_hold"},  {T_DN1_H,    T_HOLD_END,   "release"},
      {T_LATCH_R,   T_LATCH_H,    "latch_17s"}, {T_LATCH_H,  T_LATCH_END,  "latch_rel"},
   };
   ...
   for (ph& p : phases) {
      ...
      char win[24]; snprintf(win,sizeof win,"%u..%u",(unsigned)p.t0,(unsigned)p.t1);
      printf("  %-10s %-13s %4d  %6d %6d %6d %6d\n",
             p.name, win, w.count, w.first, w.last, w.lo, w.hi);
   }
   int cc=0,pc=0; for (const ev& e:events){ if(e.kind=='C')++cc; if(e.kind=='P')++pc; }
   ...
   // 2. THE check: the moving window follows each bend OUT (to ~+/-2.4 ST) and on
   // release descends through the depth-scaled hysteresis rest, so the servo nulls
   // BOTH rests statically -- even the +576 after-pull the too-tight window missed.
   ...
   snprintf(d,sizeof d,"offset=%+d (was +640 with the tight window)", r3-8192);
   ...
   // 3. real bends are preserved (reach near full). The feed-forward scales the
   //    bend by (1-k) ~ 0.957 at the peak -- the deflection that BECOMES hysteresis
   //    -- so full travel maps to ~96% of full-scale, not the rail.
   ...
   snprintf(d,sizeof d,"p-p=%d (range %+d..%+d)", wb.hi-wb.lo, wb.lo-8192, wb.hi-8192);
   check("vibrato preserved (amplitude)", (wb.hi-wb.lo) >= 700, d);

   // 5. +1 ST / -1 ST holds preserved (the SPICE +/-1 semitone hold test; the
   //    analog holds ~84% with a slow leak -- 1 ST = 683 units, 80% = 546)
   ...
   int glitch_out = pb_at(T_GLITCH + 250) - 8192;       // mid-glitch (shift held 0.5 s in rest4)
   snprintf(d,sizeof d,"mid-glitch out=%+d (held center; un-gated this latches off-center)", glitch_out);

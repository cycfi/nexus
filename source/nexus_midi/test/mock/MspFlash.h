// Host mock of Energia's MspFlash: two RAM-backed 64-byte segments that behave
// like flash (erased = 0xff, write-once-per-bit is not enforced — the firmware
// ring-buffer logic doesn't need it for functional testing).
#pragma once
#include <stdint.h>
#include <string.h>
...
struct _FlashT
{
   ...
   void write(unsigned char* dst, unsigned char* src, int n) { memcpy(dst, src, n); }
};
